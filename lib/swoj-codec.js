...
SwojCodec.prototype.deserialise = function(string, constructors, store) {
	var tmp = JSON.parse(deserialise);
	
	// Find the constructor.
	var constructor = constructors[this.wrap("constructor")];

	// And the UUID.
	var uuid = tmp[wrap("uuid")];
    ...
    ob[wrap("uuid")] = uuid; // Fix UUID.

    ownRealKeys(tmp, this.magic).forEach(function(key, i, keys) {
    	store[key] = tmp[key];
    });
}
